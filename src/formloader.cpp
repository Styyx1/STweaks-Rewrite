...
void Forms::FormLoader::LoadForms()
{
    ...
    if (Config::Settings::enable_diseases.GetValue()) {
        if (auto file = dh->LookupModByName(Forms::FormConstants::diseases_name); file && file->compileIndex != 0xFF) {
            ...
            spell_allow_list = dh->LookupForm<RE::BGSListForm>(Forms::FormConstants::spell_allow_list_ID, Forms::FormConstants::diseases_name);
            ...
            REX::FAIL(std::format("{} not found, please either enable the esp or disable the diseases in the toml file of this mod.", mod_name));
        }
    }
}
